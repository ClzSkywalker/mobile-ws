...
// NewCmd NewCmd
func NewCmd(ctx context.Context, port string) *cobra.Command {
	opts := &StartOptions{}
	...
	cmd.PersistentFlags().StringVarP(&opts.address, "address", "a", "ws://127.0.0.1"+port, "server address")
	cmd.PersistentFlags().StringVarP(&opts.user, "user", "u", "user1", "user")
	return cmd
}
